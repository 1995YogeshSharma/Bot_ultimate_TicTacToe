...
class Player3:
    ...
    #        print ""
            if i.value == state.value:
                ...
                #for j in state.child:
                 #   print j.value,
                return i.action
       # else:
            #print state.child, state.value
        #    for i in state.child :
                #print i.value , "  "
            #print "abe kamino code sudharo :P"
...
def maxValue(state, a, b):
   ...
   # state.value = -10000000
    state.child = successors(state)
    #if len(state.child) >=30:
    #    filteredChilds = filter(state.child)
    #else:
    #filteredChilds = state.child
    for i in state.child:
...
def checkCutoff(state):
   ...
   return False
   #curtime = time.time()
   #if curtime - timestart >= 10 or checkBoardFull(state.board,state.mainBlock):
   #    return True
   #return False
...
def utility(state):
    ...
    #print "--------------------------------------"
    #print state.action, state.flag
    #print "balee   ", state.mainBlock
    ...
    for i in range(0,3):
        #tempSum = probGrid[i][0] + probGrid[i][1] +probGrid[i][2]
        #mainUtil += manipulateMySum(tempSum)
    ...
    #now checking what should be added in the main utility on the basis of the sum of columnsof probGrid
    # for i in range(0,3):
    #     tempSum = probGrid[0][i] + probGrid[1][i] +probGrid[2][i]
    #     mainUtil += manipulateMySum(tempSum)

    #now checking what should be added in the main utility on the basis of the sum of diagonals of probGrid
    # tempSum = probGrid[0][0] + probGrid[1][1] + probGrid[2][2]
    # mainUtil += manipulateMySum(tempSum)

    # tempSum = probGrid[0][2] + probGrid[1][1] + probGrid[2][0]
    # mainUtil += manipulateMySum(tempSum)

    #if state.flag != maxFlag:
    #    mainUtil -= 500*getValidBlocks(state.board, state.mainBlock, state.action)[1]
 #  else:
 #      mainUtil -= 500*getValidBlocks(state.board, state.mainBlock, state.action)[1]
...
def checkBlockFill( board, block , position):
 ...
 #   print "returned from check  " , char
...
def successors(state):
   ...
   # print "printing the action " , state.action
   # print validBoards
